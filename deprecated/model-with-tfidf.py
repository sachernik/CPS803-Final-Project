...
def create_dictionary(abstracts, stopwords) :
    ...
    tfdif = np.zeros(len(dictionary))
    i = 0
    for word in dictionary :
        tfdif[i] = np.log(num_abstracts/dictionary[word])
        dictionary[word] = i
        i+=1

    return dictionary, tfdif

# consider redoing dictionary as a numpy array?
def create_matrix(abstracts, dictionary, stopwords, tfdif) :
    n = len(abstracts)
    m = len(dictionary)
...
def pre_process_train(train_data_path, dictionary_path, train_matrix_path, stopwords) :
    ...
    dictionary, tfdif = create_dictionary(abstracts, stopwords)
    matrix = create_matrix(abstracts, dictionary, stopwords, tfdif)
    ...
    return dictionary, matrix, tfdif
...
def pre_process_test(test_data_path, test_matrix_path, dictionary, stopwords, tfdif) :
    abstracts = []
...
def test_main() :
    ...
    if tiny == True :
        ...
        train_matrix_path = 'tinyoutput/train_matrix.txt'
        test_matrix_path = 'tinyoutput/test_matrix.txt'
    ...
    dictionary, train_matrix, tfdif = pre_process_train(train_data_path, dictionary_path, train_matrix_path, stopwords)
    test_matrix = pre_process_test(test_data_path, test_matrix_path, dictionary, stopwords, tfdif)
    ...
    for category in label_row_dict :
        ...
        np.savetxt(make_path(category, "pred_labels"), pred_labels, fmt='%i', delimiter=',')
